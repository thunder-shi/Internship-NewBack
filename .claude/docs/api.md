...
| /internshipProcess/getExternalInternshipStudentPostBreakdown | 校外实习学生选岗情况（视图 view_external_internship_student_post_breakdown；status=all/notSelected/selected/selectedPendingAudit/postApproved） | internshipId, status?, departmentId? |
| /internshipProcess/randomAssignPostsForUnselectedStudents | 系统分配：项目安排中未选岗学生随机报名到审核通过且有空位的企业岗位（复用 stuSelPost） | internshipId |
