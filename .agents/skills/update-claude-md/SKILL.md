...
description: 扫描当前代码库，自动更新 .Codex/AGENTS.md 中的项目统计、目录结构、实体列表、接口列表等
user_invocable: true
---

# 自动更新 AGENTS.md

扫描当前项目源码，刷新 AGENTS.md 中的所有可自动化统计的章节。
...
更新 **`.Codex/docs/entities.md`** 中的业务实体、基础实体、系统实体、视图实体四个表格（不修改主 AGENTS.md）。
...
更新 **`.Codex/docs/api.md`**（不修改主 AGENTS.md）。
...
扫描 AGENTS.md 中引用的类名和文件路径，验证它们是否仍然存在：
- 若类已不存在，标注或移除相关描述
...
- 开发规则
- `.Codex/docs/verify-flow.md`（审核流程详解，业务逻辑描述）

## 输出

直接编辑 `.Codex/AGENTS.md` 文件，仅更新可自动化的章节，保留人工编写的内容不变。更新完成后输出变更摘要。
